...
function engineRequest(
  ...
  return authenticatedFetch(`${API_URL}${path}`, init, { auth });
}
...
export async function listAuctionRegistrationsAction(auctionId: string, query: { cursor?: string; limit?: string } = {}): Promise<ActionResult<EngineAuctionRegistrationPage>> {
  try {
    ...
    const response = await engineRequest(`/auction-engine/manager/auctions/${encodeURIComponent(auctionId)}/registrations${params.size ? `?${params.toString()}` : ""}`, { cache: "no-store" });
...
export async function setAuctionRegistrationEnabledAction(auctionId: string, registrationId: string, enabled: boolean): Promise<ActionResult<EngineAuctionRegistration>> {
  try {
    const response = await engineRequest(`/auction-engine/manager/auctions/${encodeURIComponent(auctionId)}/registrations/${encodeURIComponent(registrationId)}`, { method: "PUT", headers: { "Content-Type": "application/json", "Idempotency-Key": randomUUID() }, body: JSON.stringify({ enabled }), cache: "no-store" });
...
export async function getAuctionRegistrationAction(auctionId: string): Promise<ActionResult<EngineAuctionRegistration | null>> {
	try {
		const response = await engineRequest(`/auction-engine/auctions/${encodeURIComponent(auctionId)}/registration`, { cache: "no-store" }, "optional");
...
export async function listManagerLotBidsAction(auctionId: string, lotId: string, query: EngineBidHistoryQuery = {}): Promise<ActionResult<EngineBidHistoryPage>> {
  try {
    ...
    const response = await engineRequest(`/auction-engine/manager/auctions/${encodeURIComponent(auctionId)}/lots/${encodeURIComponent(lotId)}/bids${params.size ? `?${params.toString()}` : ""}`, { cache: "no-store" });
...
export async function updateManagerBidAction(bidId: string, input: { amountCents: string; reason: string; expectedVersion?: string }): Promise<ActionResult<EngineBidManagementResult>> {
  try {
    const response = await engineRequest(`/auction-engine/manager/bids/${encodeURIComponent(bidId)}`, { method: "PATCH", headers: { "Content-Type": "application/json", "Idempotency-Key": randomUUID() }, body: JSON.stringify(input), cache: "no-store" });
...
export async function deleteManagerBidAction(bidId: string, input: { reason: string; expectedVersion?: string }): Promise<ActionResult<EngineBidManagementResult>> {
  try {
    const response = await engineRequest(`/auction-engine/manager/bids/${encodeURIComponent(bidId)}`, { method: "DELETE", headers: { "Content-Type": "application/json", "Idempotency-Key": randomUUID() }, body: JSON.stringify(input), cache: "no-store" });
...
export async function listManagerPendingBidsAction(auctionId: string): Promise<ActionResult<EnginePendingBidsPage>> {
  try {
    const response = await engineRequest(`/auction-engine/manager/auctions/${encodeURIComponent(auctionId)}/pending-bids?limit=100`, { cache: "no-store" });
...
export async function listManagerPendingEligibilityBidsAction(auctionId: string, query: { lotId?: string; limit?: string } = {}): Promise<ActionResult<EnginePendingEligibilityBidsPage>> {
  try {
    ...
    if (query.limit) params.set("limit", query.limit);
    const response = await engineRequest(`/auction-engine/manager/auctions/${encodeURIComponent(auctionId)}/pending-eligibility-bids${params.size ? `?${params.toString()}` : ""}`, { cache: "no-store" });
...
export async function approveManagerBidAction(bidRequestId: string): Promise<ActionResult<EngineBidResult>> {
  try {
    const response = await engineRequest(`/auction-engine/manager/bids/${encodeURIComponent(bidRequestId)}/approve`, { method: "POST", headers: { "Idempotency-Key": randomUUID() }, cache: "no-store" });
...
export async function rejectManagerBidAction(bidRequestId: string, reason: string): Promise<ActionResult<EngineBidResult>> {
  try {
    const response = await engineRequest(`/auction-engine/manager/bids/${encodeURIComponent(bidRequestId)}/reject`, { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": randomUUID() }, body: JSON.stringify({ reason }), cache: "no-store" });
...
async function submitBid(path: string, body: Record<string, string>): Promise<ActionResult<EngineBidResult>> {
  try {
    const response = await engineRequest(path, { method: path.endsWith("proxy-bid") ? "PUT" : "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": randomUUID() }, body: JSON.stringify(body), cache: "no-store" });
...
export async function getOwnProxyBidAction(auctionId: string, lotId: string): Promise<ActionResult<EngineOwnProxyBid>> {
	try {
		const response = await engineRequest(`/auction-engine/auctions/${encodeURIComponent(auctionId)}/lots/${encodeURIComponent(lotId)}/proxy-bid`, { cache: "no-store" }, "optional");
...
export async function createSandboxAuctionAction(input: { label?: string; lotCount?: number } = {}): Promise<ActionResult<SandboxAuction>> {
  try {
    const response = await engineRequest(`/auction-engine/sandbox/auctions`, { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": randomUUID() }, body: JSON.stringify(input), cache: "no-store" });
...
export async function issueRealtimeTicketAction(auctionId: string): Promise<ActionResult<EngineRealtimeTicket>> {
  try {
		const response = await engineRequest(`/auction-engine/auctions/${encodeURIComponent(auctionId)}/realtime/tickets`, { method: "POST", headers: { "Idempotency-Key": randomUUID() }, cache: "no-store" }, "optional");
...
export async function managerAuctionCommandAction(auctionId: string, action: string, expectedVersion?: string): Promise<ActionResult<Record<string, unknown>>> {
  try {
    const response = await engineRequest(`/auction-engine/manager/auctions/${encodeURIComponent(auctionId)}/${action}`, { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": randomUUID() }, body: JSON.stringify(expectedVersion ? { expectedVersion } : {}), cache: "no-store" });
...
export async function setAuctionParticipantEligibilityAction(userId: string, enabled: boolean): Promise<ActionResult<AuctionParticipantSearchResult>> {
  try {
    const response = await engineRequest(`/auction-engine/manager/participants/${encodeURIComponent(userId)}/eligibility`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", "Idempotency-Key": randomUUID() },
...
export async function managerStreamAction(auctionId: string, input: { provider: string; status: string; playbackUrl?: string; providerStreamId?: string }): Promise<ActionResult<EngineStream>> {
  try {
    const response = await engineRequest(`/auction-engine/manager/auctions/${encodeURIComponent(auctionId)}/stream`, { method: "PUT", headers: { "Content-Type": "application/json", "Idempotency-Key": randomUUID() }, body: JSON.stringify(input), cache: "no-store" });
