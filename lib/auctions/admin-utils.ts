...
export function calculatePauseHours(startsAt?: string | Date | null, preBidEndsAt?: string | Date | null) { const starts = startsAt ? new Date(startsAt).getTime() : NaN; const ends = preBidEndsAt ? new Date(preBidEndsAt).getTime() : NaN; const difference = starts - ends; return Number.isFinite(difference) && difference > 0 && difference % 3600000 === 0 ? difference / 3600000 : null; }
