...
import { getAuctionRegistrationAction, getEngineSnapshotAction, getOwnProxyBidAction, issueRealtimeTicketAction, placeBidAction, registerAuctionAction, reserveShoppingLotAction, setProxyBidAction } from "@/hooks/actions/auctionEngineActions";
...
function updateLot(snapshot: EngineAuctionSnapshot, event: Record<string, unknown>): EngineAuctionSnapshot {
	...
	const lotStatus = typeof event.status === "string" && ["DRAFT", "QUEUED", "OPEN", "PAUSED", "CLOSING", "SOLD", "UNSOLD", "CANCELLED"].includes(event.status) ? event.status : null;
	return {
		...snapshot,
...
function RuntimeLot({ auctionId, auction, currency, lot, registrationApproved, onRequireRegistration, onResult }: { auctionId: string; auction: EngineAuctionSnapshot["auction"]; currency: string; lot: EngineLot; registrationApproved: boolean; onRequireRegistration: () => Promise<void>; onResult: (result: ActionResult<EngineBidResult | Record<string, unknown>>) => void }) {
	...
	const reserve = async () => { if (busy) return; if (!registrationApproved) { await onRequireRegistration(); return; } setBusy(true); setPendingControl("custom"); try { const result = await reserveShoppingLotAction(auctionId, lot.externalId); onResult({ success: result.success, error: result.error, errorCode: result.errorCode, correlationId: result.correlationId }); } finally { setBusy(false); setPendingControl(null); } };
