...
function getInitialState(initialData?: AuctionAdmin): FormState {
  return initialData
    ? {
        ...
        pauseHours:
          initialData.pauseHours == null
            ? String(calculatePauseHours(initialData.startsAt, initialData.preBidEndsAt) ?? "")
            : String(initialData.pauseHours),
...
export function AuctionForm({
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (form.mode === "SHOPPING") {
      ...
      if (new Date(preBidStartsAt) >= preBidEndsAt) {
        setNotice("O início dos pré-lances deve ser anterior ao fim calculado da janela.");
        return;
      }
    }
      ...
      <form onSubmit={submit} noValidate className="space-y-5">
        <section className="rounded-xl border bg-card" aria-labelledby="auction-basic-title">
          ...
          <div className="grid gap-5 p-5 md:grid-cols-2">
            ...
            <div className="rounded-lg bg-muted/40 p-3 text-sm"><p className="font-medium">Agenda por modalidade</p><p className="mt-1 text-xs font-normal text-muted-foreground">Defina os horários na seção Agenda e regras conforme o formato escolhido.</p></div>
          ...
          <div className="space-y-5 border-t p-5">
            ...
            <section className="rounded-xl border bg-background">
              <div className="border-b px-5 py-4">
                <h2 className="font-semibold">Agenda e regras</h2>
                <p className="mt-1 text-sm text-muted-foreground">Os horários são interpretados no horário de Brasília.</p>
              </div>
              <div className="grid gap-5 p-5 md:grid-cols-2 lg:grid-cols-3">
                {form.mode === "SHOPPING" ? (
                  ...
                  <>
                    ...
                    <Field label="Incremento alternativo" id="auction-secondary">
                      <input id="auction-secondary" value={form.secondaryIncrementReais} onChange={(event) => update("secondaryIncrementReais", event.target.value)} inputMode="decimal" placeholder="Opcional" disabled={fieldDisabled} className="management-field" />
                      <span className="mt-1 block text-xs font-normal text-muted-foreground">Deixe vazio para usar apenas o padrão.</span>
                    </Field>
                    <Field label="Prorrogação (minutos)" id="auction-extension">
